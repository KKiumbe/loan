...
async function checkUserAuthorization(req, loan) {
...
async function performDisbursement(loan, userId) {
    ...
    const result = await (0, initiateB2CPayment_1.disburseB2CPayment)({
        phoneNumber,
        amount: loan.amount,
        loanId: loan.id,
        userId,
...
async function sendDisbursementFailureAudit(loan, userId, reason) {
...
async function notifyUserDisbursementFailure(loan, reason) {
    const tenant = await prisma.tenant.findUnique({ where: { id: loan.tenantId }, select: { name: true } });
    await (0, sms_1.sendSMS)(loan.tenantId, loan.user.phoneNumber, `Dear ${loan.user.firstName}, your loan of KES ${loan.amount} at ${tenant?.name} could not be disbursed ${reason}.`);
}
async function approveStep(loan, userId) {
    const newCount = loan.approvalCount + 1;
    let updateData = { approvalCount: newCount };
    if (loan.organization.approvalSteps === 1 || newCount === 2) {
        updateData.status = 'APPROVED';
    }
    if (newCount === 1)
        updateData.firstApproverId = userId;
    if (newCount === 2)
        updateData.secondApproverId = userId;
...
const approveLoan = async (req, res) => {
    ...
    try {
        if (!id)
            throw { status: 400, message: 'Loan ID is required' };
        ...
        if (loan.status !== 'PENDING')
            throw { status: 400, message: 'Loan is not in PENDING status' };
        ...
        if (updatedLoan.status === 'APPROVED') {
            ...
            const interestDescription = loan.organization?.interestRateType === 'DAILY'
            ...
            await (0, sms_1.sendSMS)(loan.tenantId, loan.user.phoneNumber, message);
        }
        await prisma.auditLog.create({
            data: {
                tenant: { connect: { id: loan.tenantId } },
